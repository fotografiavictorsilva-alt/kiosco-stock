...
export default function SalesHistory({ refreshTrigger, triggerRefresh, showToast }) {
  ...
  const filteredSales = sales.filter(sale => {
    ...
    if (dateFilter === 'today') {
      ...
      const diffDays = (todayDate - saleDate) / (1000 * 60 * 60 * 24);
  ...
  const handleDeleteSale = (sale) => {
    const confirmReturnStock = window.confirm(
      `Estas seguro de anular la venta "${sale.id}"?\n\nPresiona ACEPTAR para reintegrar los productos al stock del inventario, o CANCELAR para anular sin devolver los productos.`
    );
    ...
    if (confirmReturnStock) {
      sale.items.forEach(item => {
        try {
          ...
          console.error('Producto no encontrado al reintegrar stock:', e);
        }
      });
    ...
    <div className="animation-fade" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div className="section-header">
        <div>
          ...
          <p className="section-subtitle">Visualiza tus facturaciones, margenes de ganancia y rendimiento</p>
        </div>
      </div>
      ...
      <div className="metrics-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
        ...
        <div className="card metric-card secondary" style={{ padding: '1.25rem' }}>
          <div className="metric-info">
            <span className="metric-label" style={{ fontSize: '0.8rem' }}>Facturacion Total</span>
        ...
        <div className="card metric-card success" style={{ padding: '1.25rem' }}>
          ...
          <div className="metric-icon-container" style={{ width: '2.5rem', height: '2.5rem' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="12 2 2 7 12 12 22 7 12 2" /><polyline points="2 17 12 22 22 17" /></svg>
          </div>
        </div>
      </div>
      ...
      <div className="card filter-row">
        <div style={{ flex: 1, minWidth: '150px' }}>
          <label className="form-label" style={{ fontSize: '0.75rem', marginBottom: '0.25rem', display: 'block' }}>Periodo de Tiempo</label>
        ...
        <div style={{ flex: 1, minWidth: '150px' }}>
          <label className="form-label" style={{ fontSize: '0.75rem', marginBottom: '0.25rem', display: 'block' }}>Metodo de Pago</label>
      ...
      <div className="card">
        {filteredSales.length === 0 ? (
          <p style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-secondary)' }}>
            No se registran ventas en el periodo e filtros seleccionados.
          </p>
        ) : (
            ...
            <table className="custom-table">
              ...
              <tbody>
                {filteredSales.map(sale => (
                  <tr key={sale.id}>
                    <td style={{ fontFamily: 'monospace', fontSize: '0.85rem', fontWeight: 600 }}>{sale.id}</td>
                    <td>{new Date(sale.timestamp).toLocaleString('es-AR')}</td>
                    <td>
                      <span className={`badge ${sale.paymentMethod === 'Efectivo' ? 'badge-success' : sale.paymentMethod === 'Tarjeta' ? 'badge-info' : 'badge-warning'}`}>
                        {sale.paymentMethod}
                      </span>
                    </td>
                    <td style={{ textAlign: 'center', fontWeight: 'bold' }}>{sale.items.reduce((acc, i) => acc + i.quantity, 0)}</td>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                        <button onClick={() => handleOpenDetailModal(sale)} className="btn btn-ghost btn-sm" style={{ padding: '0.25rem 0.5rem' }}>Detalle</button>
                        <button onClick={() => handleDeleteSale(sale)} className="btn btn-danger btn-sm" style={{ padding: '0.25rem 0.5rem' }}>Anular</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <div>
                <h2 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Detalle de Transaccion</h2>
                <span style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{selectedSale.id}</span>
              </div>
              <button onClick={() => setIsDetailModalOpen(false)} className="btn btn-ghost btn-circle btn-sm"><Icons.Close /></button>
            </div>
            <div className="modal-body">
              ...
              <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.5rem', borderBottom: '1px solid var(--card-border)', paddingBottom: '0.25rem' }}>Articulos Vendidos</h3>
              ...
              <div style={{ background: 'rgba(0,0,0,0.15)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <div className="modal-footer">
              <button onClick={() => setIsDetailModalOpen(false)} className="btn btn-primary" style={{ width: '100%' }}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
